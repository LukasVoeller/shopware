...
class ProductExportGenerator implements ProductExportGeneratorInterface
{
    ...
    public function generate(ProductExportEntity $productExport, ExportBehavior $exportBehavior): ?ProductExportResult
    {
        ...
        // own sorting via ProductExportProductCriteriaEvent it falls back to resumable offset
        // pagination. Either way $offset is the opaque resume position carried across batches.
        $offset = $exportBehavior->offset();
        ...
        while ($result !== null) {
            $fetched = $result->getEntities()->count();
        ...
        $errors = $exportBehavior->batchMode()
            ? []
